 ...
 * @author loup626
 */
public class BonusRalentirBalle extends ABonus {

    public static ArkanoidRebours rebour;

    public BonusRalentirBalle(ArkanoidJPanel pere) {
        super(pere, null, 0, 0, 0);
    }

    public BonusRalentirBalle(ArkanoidJPanel pere, ArkanoidTimer timer, int x, int y) {
    ...
    @Override
    public void action() {
        if (!this.pere.estAccelere()) {
            BonusRalentirBalle.rebour.initialiser();
        } else {
            BonusAccelerationBalle.rebour.cancel();
        }

        this.pere.ralentirBalles();
    }

    public void annuler() {
        this.pere.accelererBalles();
    }
}
