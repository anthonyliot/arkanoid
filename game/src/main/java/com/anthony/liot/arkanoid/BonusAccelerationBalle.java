 ...
 * @author loup626
 */
public class BonusAccelerationBalle extends ABonus {

    public static ArkanoidRebours rebour;

    public BonusAccelerationBalle(ArkanoidJPanel pere) {
        super(pere, null, 0, 0, 0);
    }

    public BonusAccelerationBalle(ArkanoidJPanel pere, ArkanoidTimer timer, int x, int y) {
    ...
    public void action() {
        if (!pere.estRalenti()) {
            BonusAccelerationBalle.rebour.initialiser();
        } else {
            BonusRalentirBalle.rebour.cancel();
        }
